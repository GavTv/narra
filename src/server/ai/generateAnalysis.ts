...
export async function generateAIAnalysis(
  ...
  const content = await generateWithLLM({
    ...
    prompt: [
      ...
      "summary — 2–3 предложения на русском. НЕ повторяй цифры из метрик-карточек (всего записей, пик, среднее по главному показателю). Вместо этого: период/состав отчёта, лидер по категории (товар/регион) с долей, второй разрез или вывод. Только из данных. Без воды и без перечисления всех колонок.",
